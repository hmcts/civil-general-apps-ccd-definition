...
Feature('CCD 1v1 - General Application Journey @mm');

Scenario.skip('Create Single general application for 1v1 and respond to application', async ({I, api}) => {
...
Scenario.only('Create Multiple general applications for 1v1', async ({I, api}) => {
  caseNumber = await api.createClaimWithRepresentedRespondent(
    config.applicantSolicitorUser, mpScenario);
  console.log('Case created for general application: ' + caseNumber);
  await I.login(config.applicantSolicitorUser);
  ...
  childCaseId = await I.grabChildCaseNumber();
  await I.navigateToCaseDetails(childCaseNum());
  await I.judgeRequestMoreInfo('requestMoreInfo', 'requestMoreInformation');
}).retry(1);
