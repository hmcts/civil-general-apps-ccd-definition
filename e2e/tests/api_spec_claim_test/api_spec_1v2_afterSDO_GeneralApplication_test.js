...
Scenario('Spec Claimant create GA - JUDICIAL_REFERRAL state @mm', async ({api}) => {
 civilCaseReference = await api.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, mpScenario);
  ...
  await api.defendantResponseSpecClaim(config.secondDefendantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_TWO');
